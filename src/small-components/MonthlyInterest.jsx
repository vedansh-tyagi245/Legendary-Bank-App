...
function MonthlyInterest() {
    ...
    const [sipAmount, setSipAmount] = useState(10000); // SIP Monthly Amount
    const [lumpsumAmount, setLumpsumAmount] = useState(5000); // Lumpsum Initial Amount

    // Separate interest rates for SIP and Lumpsum
    const [sipRate, setSipRate] = useState(5); // SIP Interest Rate (monthly)
    const [lumpsumRate, setLumpsumRate] = useState(5); // Lumpsum Interest Rate (monthly)
    ...
    const calculateSIP = () => {
        const r = sipRate / 100;
        const monthlyRate = r / 12; // Monthly rate (dividing by 12 for monthly interest)
        const months = sipTime * 12; // Convert years to months
        const futureValueSIP = sipAmount * ((Math.pow(1 + monthlyRate, months) - 1) / monthlyRate) * (1 + monthlyRate);
        setResultSIP(futureValueSIP.toFixed(2));
    };

    const calculateLumpsum = () => {
        const r = lumpsumRate / 100;
        const futureValueLumpsum = lumpsumAmount * Math.pow(1 + r / 12, lumpsumTime * 12); // Monthly compounding
